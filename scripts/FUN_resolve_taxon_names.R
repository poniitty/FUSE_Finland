# orig_name <- "Chenopodium murale L. var. (mixed with album)"
      ...
      for(tryn in 1:maxtry){
        if(is.null(NAME)){
          try({
            i <- orig_name
            nl <- gbif_parse(i)
            if("canonicalname" %in% names(nl)){
              if(length(strsplit(nl$canonicalname," ")[[1]]) < 2){
                i <- nl$canonicalname
              }
            }
            ...
            if(length(parts) > 1){
              ...
              if(parts[2] %in% c("sp","sp.","spp","ssp","spp.","ssp.","spec.","species","s.")){
                i <- parts[1]
              }
            }
            if(grepl(" = ",i)){
            ...
            if(grepl("cespitosa", i) & nrow(nl) == 0){
              ii <- gsub("cespitosa","caespitosa",i)
              nl <- name_suggest(q=ii, datasetKey = dataset_key)$data
              if(nrow(nl) > 0){
                i <- ii
              } else {
                nl <- name_suggest(q=i, datasetKey = dataset_key)$data
              ...
              if(grepl("caespitosa", i) & nrow(nl) == 0){
                ii <- gsub("caespitosa","cespitosa",i)
                nl <- name_suggest(q=ii, datasetKey = dataset_key)$data
                if(nrow(nl) > 0){
                  i <- ii
                } else {
                  ...
                  if(gbif_parse(i)$canonicalname %in% nl$canonicalName){
                    ...
                    if(nrow(nl) == 1){
                      ...
                      if(nrow(nl) == 1){
                        ...
                        if(length(unique(nl$scientificName)) == 1){
                          nl <- nl[1,]
                          if(!nl$synonym){
                            if(nl$taxonomicStatus == "ACCEPTED"){
                              NAME <- nl$scientificName
            ...
            if(is.null(NAME) | NOTE == "PARSED TO SPECIES"){
              ...
              if((length(strsplit(i, " ")[[1]]) == 1 & i == tp$New.Genus)){
                ...
                KEY <- NA
                STATUS <- NA
                NOTE = "NOT FOUND! LIKELY HIGHER LEVEL TAXON"
                RANK <- NA
                SPEC_NAME <- NA
              } else {
